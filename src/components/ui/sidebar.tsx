...
>(
  (
    ...
    return (
      <div
        ref={ref}
        className="group peer hidden md:flex flex-col h-svh bg-sidebar text-sidebar-foreground transition-[width] duration-300 ease-in-out"
        data-state={state}
      ...
      {React.Children.map(props.children, (child) => {
        if (React.isValidElement(child) && child.type === SidebarMenuSub) {
          ...
          return React.cloneElement(child as React.ReactElement<any>, {
            style: { 
              display: (isOpen && state === 'collapsed') || state === 'expanded' ? 'block' : 'none',
              position: state === 'collapsed' ? 'absolute' : 'relative',
              left: state === 'collapsed' ? '100%' : 'auto',
              top: state === 'collapsed' ? 0 : 'auto',
...
>(
  (
    ...
    const content = asChild ? children : <span className="flex w-full items-center gap-3">{children}</span>
  ...
  return (
    <ul
      ...
      className={cn(
        ...
        state === "expanded" && "mx-3.5 border-l border-sidebar-border px-2.5 py-0.5",
        className
      )}
      {...props}
    />
